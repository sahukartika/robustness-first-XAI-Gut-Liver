"""
metabolomics_shap_analysis_single_config_v2.py

SHAP ANALYSIS FOR METABOLOMICS 1-FILE PIPELINE (SINGLE CONFIG)
- Matches comprehensive_metabolomics_pipeline.py (1-file version)
- You provide:
    * results_file: path to all_results.csv from the main pipeline
    * data_file:    metabolomics data file (features in first column, samples as columns)
    * label_file:   labels file
    * config_name:  EXACT config_name to analyze
    * model_name:   EXACT model_name to analyze

Script will:
    * Rebuild X, y, feature_names from original data + labels
...
# ============================================================================
# CONFIG PARSER (MATCH MAIN PIPELINE NAMING)
# ============================================================================

def parse_metabolomics_config(config_name: str) -> Dict[str, Any]:
    """
    Parse metabolomics config name like:
...
def parse_best_params(params_str: str) -> Dict[str, Any]:
    """
...
def normalize_shap_output(shap_values, n_samples: int, n_features: int) -> np.ndarray:
    """Normalize SHAP output into (n_samples, n_features) for binary classification."""
    ...
    if shap_values.ndim == 3:
        ...
        if shap_values.shape[2] == 2:
            return shap_values[:, :, 1]
        # Fallback: pick last along smallest dimension
...
def compute_shap_all_samples(
    ...
    try:
        ...
        if model_name in ['RandomForest', 'XGBoost', 'LightGBM']:
            ...
            shap_values = normalize_shap_output(shap_raw, n_samples_shap, n_features_shap)
        else:
            # KernelExplainer for non-tree models (can be slow)
            if n_samples_shap > 200:
                background = shap.sample(X_transformed, 200)
            else:
        ...
        print(f"      [OK] SHAP: {n_samples_shap} samples, {len(feature_indices)} selected features")
        return shap_df
...
def run_cv_evaluation(
    ...
    for tr_idx, te_idx in skf.split(X, y):
        ...
        pipeline = build_metabolomics_pipeline(config_dict, model, ytr)
...
# ============================================================================
# DATA LOADER (1 FILE) - MATCH MAIN PIPELINE
# ============================================================================
...
def load_metabolomics_data(
    ...
    """
    Load single file (data.xlsx) where:
    - First column: feature names (may include metadata rows like 'retention index', 'm/z')
    - Remaining columns: sample IDs (each column is a sample)
...
def analyze_metabolomics_results_single_config(
    ...
    data_file: str,
    label_file: str,
    output_folder: str = "metabolomics_shap_single_config",
    validate: bool = False,
    ...
    """
    Run SHAP analysis for a SINGLE configuration+model from the
    1-file metabolomics pipeline.

    Parameters:
        results_file: path to all_results.csv from main pipeline
        data_file:    metabolomics data file (features × samples)
        label_file:   labels file
        output_folder: where to save SHAP outputs
        validate: whether to run CV validation with permutation test and bootstrap CI
        config_name: exact config_name to analyze (required)
        model_name: exact model_name to analyze (required)
    ...
    """
    print(f"\n{'='*80}")
    print("METABOLOMICS SHAP ANALYSIS (1-FILE) - SINGLE CONFIG")
    print(f"{'='*80}")
    print("- Uses results from comprehensive_metabolomics_pipeline.py (1-file)")
    print("- Rebuilds X, y, features from original data + labels")
    ...
    if config_name is None or model_name is None:
        raise ValueError("You must provide both config_name and model_name.")
    ...
    # Filter for specified config/model
    ...
    print(f"\n{'='*80}")
    print("METABOLOMICS SHAP ANALYSIS COMPLETE (1-FILE, SINGLE CONFIG)")
    print(f"{'='*80}")
...
if __name__ == "__main__":
    # 1) Path to all_results.csv from main 1-file metabolomics pipeline
    RESULTS_FILE = r"CRC_Metabolomics_results\all_results.csv"  # ← UPDATE

    # 2) Data & label files (same as main pipeline)
    DATA_FILE = r"CRC_Metabolomics_Data.xlsx"       # ← UPDATE
    LABEL_FILE = r"CRC_Metabolomics_label.xlsx"     # ← UPDATE

    # 3) Output folder
    OUTPUT_FOLDER = "shap_analysis_CRC_metabolomics"

    # 4) Config and model to analyze (must match a row in all_results.csv)
    CONFIG_TO_ANALYZE = "standard_random_forest_56f(50%)_NoBalance"   # ← UPDATE
    MODEL_TO_ANALYZE = "XGBoost"                              # ← UPDATE

    # 5) Permutation test and bootstrap CI settings (only used when validate=True)
